...
public class Minecraft implements IThreadListener, ISnooperInfo {
	...
	private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
		...
		vertexbuffer.pos((double) this.displayWidth, (double) this.displayHeight, 0.0D).tex(0.0D, 0.0D)
		...
		if (!list.isEmpty()) {
			...
			if (keyCount == 0) {
				...
				if (keyCount < list.size()
						&& !"unspecified".equals(((Profiler.Result) list.get(keyCount)).profilerName)) {
					if (!this.debugProfilerName.isEmpty()) {
						this.debugProfilerName = this.debugProfilerName + ".";
					}

					this.debugProfilerName = this.debugProfilerName
							+ ((Profiler.Result) list.get(keyCount)).profilerName;
				}
			}
		}
	}
	...
	private void displayDebugInfo(long elapsedTicksTime) {
		if (this.mcProfiler.profilingEnabled) {
			...
			vertexbuffer.pos((double) ((float) j - 176.0F), (double) ((float) k - 96.0F - 16.0F), 0.0D)
			...
			for (int l = 0; l < list.size(); ++l) {
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					...
					vertexbuffer.pos((double) ((float) j + f1), (double) ((float) k - f2), 0.0D).color(k1, l1, i2, 255)
							.endVertex();
				}
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					...
					vertexbuffer.pos((double) ((float) j + f4), (double) ((float) k - f5), 0.0D)
							.color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
					vertexbuffer.pos((double) ((float) j + f4), (double) ((float) k - f5 + 10.0F), 0.0D)
							.color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
				}
	...
	private boolean processKeyF3(int p_184122_1_) {
		if (p_184122_1_ == 30) {
			...
			this.debugFeedbackTranslated("debug.cycle_renderdistance.message",
					new Object[] { Integer.valueOf(this.gameSettings.renderDistanceChunks) });
			return true;
			...
			this.debugFeedbackTranslated(this.gameSettings.advancedItemTooltips ? "debug.advanced_tooltips.on"
					: "debug.advanced_tooltips.off", new Object[0]);
			this.gameSettings.saveOptions();
			return true;
	...
	public void setDimensionAndSpawnPlayer(int dimension) {
		...
		this.player = this.playerController.createClientPlayer(this.world,
	...
	public MusicTicker.MusicType getAmbientMusicType() {
		return this.player != null
				? (this.player.world.provider instanceof WorldProviderHell ? MusicTicker.MusicType.NETHER
						: (this.player.world.provider instanceof WorldProviderEnd
								? (this.ingameGUI.getBossOverlay().shouldPlayEndBossMusic()
										? MusicTicker.MusicType.END_BOSS : MusicTicker.MusicType.END)
								: (this.player.capabilities.isCreativeMode && this.player.capabilities.allowFlying
