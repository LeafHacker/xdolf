...
public class CrystalAura extends Module {
	...
	public void tick() {
		for (Entity e : Wrapper.getWorld().loadedEntityList) {
			if (Wrapper.getPlayer().getDistanceToEntity(e) > 5 && e != Wrapper.getPlayer() && Wrapper.getPlayer().canEntityBeSeen(e)) {
				placeCrystal();
		...
		for(int slot = 44; slot >= 9; slot--) {
			...
			if(stack != null) {
				if(slot >= 36 && slot <= 44) {
					...
					Wrapper.getMinecraft().playerController.windowClick(0, slot, 0, ClickType.PICKUP_ALL, Wrapper.getPlayer());
					Wrapper.getMinecraft().playerController.windowClick(0, currentSlot, 0, ClickType.SWAP, Wrapper.getPlayer());
					for(slot = 44; slot >= 9; slot--) {
						if(Wrapper.getPlayer().inventoryContainer.getSlot(slot).getStack() == null)
						{
							Wrapper.getMinecraft().playerController.windowClick(0, slot, 0, ClickType.SWAP, Wrapper.getPlayer());
						}
					}
				}
			}
